...
Start Menu shortcuts. If that exact version is already installed, the
download/copy is skipped and only the shortcuts are (re)ensured.

How to run:
1) Run PowerShell as Administrator.
2) Execute: .\Get-LatestGodot.ps1
...
$log_path	= Join-Path $log_dir "godot_latest_install.log"
$stage_dir	= Join-Path $env:TEMP "Godot_Latest_Download"
$headers	= @{ 'User-Agent' = 'Get-LatestGodot-Script' }
...
# Skip the download/copy entirely if this version is already fully installed
$alreadyInstalled = $true
foreach ($v in $variantsToInstall) {
	$destDir = Join-Path $versionRoot $v
	$exe = if (Test-Path -LiteralPath $destDir) {
		Get-ChildItem -LiteralPath $destDir -Filter '*.exe' -Recurse -ErrorAction SilentlyContinue | Where-Object { $_.Name -notmatch '_console\.exe$' } | Select-Object -First 1
	}
	if (-not $exe) { $alreadyInstalled = $false }
}

$installedExes = @{}

if ($alreadyInstalled) {
	log "Godot $version is already installed at $versionRoot for the requested variant(s); skipping download."
	foreach ($v in $variantsToInstall) {
		$destDir = Join-Path $versionRoot $v
...
foreach ($v in $installedExes.Keys) {
	...
	log "Ensuring Desktop shortcut $lnkPath"
